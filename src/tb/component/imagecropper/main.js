...
define(
    ...
    function (Core,
        ...
        var ImageCropper = new JS.Class({
            ...
            show: function (imageUid, imagePath, originalName, element) {
                var self = this,
                    imageUrl = '',
            ...
            onOpen: function (self) {
                ...
                cropNewBtn.on('click', function () {
                    CropRepository.postData({
                        ...
                        self.onDone(self, response, 'new_cropimage_created');
                    }).fail(function () {
                    ...
                    var config = {
                            validateCallback: function () {
                                CropRepository.postData({
                                    ...
                                    self.onDone(self, response, 'image_was_cropped');
                                }).fail(function () {
                                    require('component!notify').error(Translator.translate('invalid_element'));
                                });
                            }
            ...
            onDone: function (self, imagePath, label) {
                var contentConfig = {},
                    contentImage,
                    ...
                    file = {'name': self.originalName},
                    value = {
                        thumbnail: baseUrl + 'images/' + imagePath,
                        name: self.originalName,
                        path: imagePath
                    };

                require('component!notify').success(Translator.translate(label));
                self.popin.destroy();
                // remove the previous thumbnail and add the new one
                if (items.length) {
                    items.first().remove();
                }
                dropzone.options.addedfile.call(dropzone, file);
                dropzone.createThumbnailFromUrl(file, value.thumbnail + '?' + new Date().getTime(), null, 'Anonymous');
                if (jQuery('.ui-dialog.media-library').length > 0) {
                    jQuery('.ui-dialog:last').find('.ui-dialog-content input[name=' + self.element.getKey() + ']').val(imagePath);
                    jQuery('.ui-dialog:last').find('.ui-dialog-content button.bb-submit-form').click();
                } else {
                    jQuery('.ui-dialog .ui-dialog-content').has('input[name=' + self.element.getKey() + ']').dialog('destroy').remove();
                    ...
                    contentImage = new AbstractContent(contentConfig);
